...
public class DogFragment extends Fragment {
    ...
    public static DogFragment newInstance(int scrollState, ArrayList<Datum> datum)
    {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        recyclerView=(RecyclerView)view.findViewById(R.id.rv_dog);
    ...
    private void fillData()
    {
        if(InternetConnection.checkConnection(getActivity().getBaseContext())) {
            ...
            MyApp.getApi().getList("dog").enqueue(new Callback<Model>() {
                @Override
                public void onResponse(Call<Model> call, Response<Model> response) {
                    final Model model=new Model();
                    model.setData(response.body().getData());
                    datumList=model.getData();
                    adapter=new Adapter(datumList,getActivity().getApplicationContext());
                    recyclerView.setAdapter(adapter);
                    recyclerView.getAdapter().notifyDataSetChanged();
                }
